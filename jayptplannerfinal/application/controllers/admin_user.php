	...
	class Admin_user extends CI_Controller
	{
		...
		public function index()
		{
		 ...
		 public function update()
		 {
			...
			$data_email=$this->user_model->email_verify_except_own($id,$data);
			if(!$data_email)
			{
			...
			else
			{
				...
				if($_FILES['image']['name']!='')
				{
				$data=array('name'=>$name,'picture'=>$val,'address'=>$address,'company'=>$company,'work_address'=>$work,'billing_address'=>$billing,'phone'=>$phone,'status'=>$status);
				$this->user_model->update($id,$data);
				$this->session->set_flashdata('flash_message','page_update');
				redirect('control/user');
				}
				...
				{
					$data=array('name'=>$name,'address'=>$address,'company'=>$company,'work_address'=>$work,'billing_address'=>$billing,'phone'=>$phone,'status'=>$status);
				$this->user_model->update($id,$data);
				$this->session->set_flashdata('flash_message','page_update');
				redirect('control/user');
