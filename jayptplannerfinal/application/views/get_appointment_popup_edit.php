...
$booking_details = $ci->common_model->get('user_booking',array('*'),$where_booking);
?>
<script>
...
<form action="<?php echo base_url(); ?>dashboard/edit_booking" method="post" name="appoit_form">
   <input type="hidden" name="mode" value="edit_appointment">
...
<input type="hidden" name="slot_another" id="slot_another" value="">
    <div class="appoint_pop">
      ...
      <h2 class="pop_heading">Change Booked appointment</h2>
      	...
      	<span>
            ...
            ?>
      		<select class="select_box" name="client_name">
                    ...
                    {
                        ?>
                        <option value="<?php echo $user['id']; ?>" <?php if($booking_details[0]['client_id'] == $user['id']) echo 'selected'; ?>>
                            <?php
                            echo $user['name'];
                            ?>
                        </option>
      ...
      <a href="javascript:void(0)" onclick="edit_book_details()" class="confirm_btn">Update Booking</a>
      </div>
</form>